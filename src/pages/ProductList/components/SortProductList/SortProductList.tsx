...
export default function SortProductList({ queryConfig, pageSize }: Props) {
  ...
  const isActiveSortBy = (value: Exclude<ProductConfig['sort_by'], undefined>) => {
    return sort_by === value
  }

  const handleSort = (value: Exclude<ProductConfig['sort_by'], undefined>) => {
    navigate({
      pathname: path.home,
  ...
  const handleOrderPrice = (value: Exclude<ProductConfig['order'], undefined>) => {
    navigate({
      pathname: path.home,
      search: createSearchParams({
        ...queryConfig,
      ...
      <div className='flex flex-wrap items-center justify-between gap-2'>
        <div className='flex flex-wrap items-center gap-2'>
          ...
          <select
            ...
            onChange={(e) => handleOrderPrice(e.target.value as Exclude<ProductConfig['order'], undefined>)}
          >
            <option className='bg-white text-black' value='' disabled>
              Giá
            </option>
            <option className='bg-white text-black' value={orderByPrice.asc}>
              Giá: Thấp đến cao
            </option>
            <option className='bg-white text-black' value={orderByPrice.desc}>
              Giá: Cao đến thấp
            </option>
          </select>
        </div>
